...
@section('konten')
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('folder-galeri.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <label for="imam_id" class="form-label">Nama Folder</label>
                        <input type="text" class="form-control" name="nama" id="exampleInputEmail1"
                            aria-describedby="emailHelp" placeholder="Masukkan Nama Folder" required>
                        <br>
                        ...
                        <select required name="kategori_galeri_id" id="select" class="form-control">
        ...
        <div style="height:90%; margin-top: 3.5rem;"class="modal fade " id="editModal{{ $d->id }}" tabindex="-1"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('folder-galeri.update', $d->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            <label for="imam_id" class="form-label">Nama Folder</label>
                            <input type="text" class="form-control" name="nama" id="exampleInputEmail1"
                                aria-describedby="emailHelp" placeholder="Masukkan Nama Program" value="{{ $d->nama }}"
                                required>
                            <br>
                            ...
                            @if (!empty($d->thumbnail))
                                <img src="{{ url('admin/assets/img/galeri/folder') }}/{{ $d->thumbnail }}"
                                    alt="project-image" class=" mt-1" style="width: 250px;height:150px">
                            @endif
                            <br>
                            <label for="kategori_galeri_id" class="form-label">Folder</label>
                            <select required name="kategori_galeri_id" id="select" class="form-control">
                                <option value="" disabled selected>Pilih Kategori</option>
                                @foreach ($kategori as $k)
                                    <option value="{{ $k->id }}"
                                        {{ $d->kategori_galeri_id == $k->id ? 'selected' : '' }}>
                                        {{ $k->nama }}
                                    </option>
                                @endforeach
                            </select>
    ...
    <div class="container-fluid px-4">
        <h1 class="mt-4">Data Folder Galeri</h1>
                        ...
                        <tbody>
                            ...
                            @foreach ($data as $d)
                                <tr>
                                    ...
                                    <td>{{ $d->nama }}</td>
                                    <td>
                                        <img src="/admin/assets/img/galeri/folder/{{ $d->thumbnail }}"
                                    ...
                                    <td>
                                        ...
                                        <!-- Modal -->
                                        <div class="modal fade notif-hapus apus" id="exampleModal{{ $d->id }}"
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        <form action="{{ route('folder-galeri.destroy', $d->id) }}"
