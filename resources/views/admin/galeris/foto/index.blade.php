...
@section('konten')
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('galeri.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <label for="imam_id" class="form-label">Nama Foto</label>
                        <input type="text" class="form-control" name="nama" id="exampleInputEmail1"
                            aria-describedby="emailHelp" placeholder="Masukkan Nama Foto" required>
                        <br>
                        <label for="tanggal" class="form-label">Foto</label>
                        <input id="text5" name="foto" type="file" class="form-control" required>
                        <br>
                        <label for="folder_galeri_id" class="form-label">Folder</label>
                        <select required name="folder_galeri_id" id="select" class="form-control">
        ...
        <div style="height:90%; margin-top: 3.5rem;"class="modal fade " id="editModal{{ $d->id }}" tabindex="-1"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{ route('galeri.update', $d->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="modal-body">
                            <label for="imam_id" class="form-label">Nama Foto</label>
                            ...
                            <input id="text5" name="foto" type="file" class="form-control"
                                value="{{ $d->foto }}">
                            @if (!empty($d->foto))
                                <img src="{{ url('admin/assets/img/galeri/foto') }}/{{ $d->foto }}" alt="project-image"
                            ...
                            <select required name="folder_galeri_id" id="select" class="form-control">
                                ...
                                @foreach ($folder as $k)
                                    <option value="{{ $k->id }}"
                                        {{ $d->folder_galeri_id == $k->id ? 'selected' : '' }}>
                                        {{ $k->nama }}
                                    </option>
                                @endforeach
                            </select>
    ...
    <div class="container-fluid px-4">
        <h1 class="mt-4">Data Foto Galeri</h1>
                        ...
                        <tbody>
                            ...
                            @foreach ($data as $d)
                                <tr>
                                    ...
                                    <td>{{ $d->nama }}</td>
                                    <td>
                                        <img src="/admin/assets/img/galeri/foto/{{ $d->foto }}"
                                            alt="{{ $d->nama }}" class="img-fluid rounded" style="max-width: 80px;">
                                    </td>
                                    <td>{{ $d->folder }}</td>
                                    <td>
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-footer">
                                                        ...
                                                        <form action="{{ route('galeri.destroy', $d->id) }}" method="POST"
